...
class Bot(Bot):
    def __init__(self):
        ...
        async def play(ctx: commands.Context, play_text: str):
            ...
            try:
                ...
                await self.send_exceptions(str(traceback.format_exc()), 'Play')
                embed_msg = Embed(
                    title="ERRO",
        ...
        #     await ctx.respond('A música foi pausada.', delete_after=self.delete_time)
        ...
        #     await ctx.respond('A música foi resumida.', delete_after=self.delete_time)
        ...
        #     await ctx.respond(f"Pong! ({lat}ms)", delete_after=self.self.delete_time)
    ...
    async def send_exceptions(self, exception: str, command: str = None) -> None:
